...
int panduan_huosi(int p,int q,int c);	//判断是否形成活四；
int panduan_chongsi(int p,int q,int c);	//判断是否形成冲四；
int panduan_huosan(int p,int q,int c);	//判断是否形成活三；
int panduan_huoer(int p,int q,int c);	//判断是否形成活二；
...
int main()
{
	...
	for(k=0;k<MAXLEN;k++){
		...
		if(c==0){	//c=0表示先手；
			...
			if(win(p,q,1) && (changlianjinshou(p,q,1)!=1)){	//判断输赢；
				printf("BLACK WIN\n");
				return 0;
			}
			if(changlianjinshou(p,q,1) || sisijinshou(p,q,1) || shuanghuosanjinshou(p,q,1)){	//判断禁手；
				printf("WHITE WIN\n");
				return 0;
			}
			c=1;		
			...
			n1=getchar();	n2=getchar();	//用getchar函数从键盘上读入坐标；
			if(n1=='q'){	
				return 0;
			}
			if(n2!=' '){
...
void recordtoDisplayArray(void){
	int m,n;	//第一步：将aInitDisplayBoardArray中记录的空棋盘，复制到aDisplayBoardArray中
	for(m=0;m<SIZE;m++){
	...
	for(i=1;i<=SIZE;i++){
		for(j=1;j<=SIZE;j++){
			if(aRecordBoard[i][j] == 1){
				for(k=0;k<=1;k++){
					aDisplayBoardArray[SIZE-i][(j-1)*CHARSIZE+k]=play1Pic[k];
				}			
			}
			else if(aRecordBoard[i][j] ==2){
				for(k=0;k<=1;k++){
					aDisplayBoardArray[SIZE-i][(j-1)*CHARSIZE+k]=play2Pic[k];
				}
			}
		}
	}
...
void displayBoard(void){	//显示棋盘格局 
	...
	for(i=0;i<SIZE;i++){
		...
		for(j=0;j<SIZE*CHARSIZE+1;j++){//将aDisplayBoardArray输出到屏幕上
...
int win(int a,int b,int c)	//判断输赢；
{ 
	int n=1,m,P,Q;
	P=a; Q=b; 
	while(n!=5){ 
...
void yiwei(int n,int *i,int *j)	//以（i,j）为中心，绕上下、左右、左斜、右斜四个方向扫描；
{ 
	switch(n){ 
	case 1: *i+=1; break;
...
int changlianjinshou(int a, int b,int c)	//判断长连禁手，以（i，j）为中心的四个方向连续超过5个即为长连；
{
	int n=1,m,P,Q;
...
//以后的循环情况与此函数的循环情况基本相同，不再赘述;
	int n=1,m,P,Q,huosan=0,liangduan1,liangduan2,temp,kong;	//liangduan1，liangduan2用来判断是活三还是冲三；
	...
	while(n!=5){ 
		m=0; liangduan1=-1;	liangduan2=-1;	temp=0;	kong=0;	chenghuosi=0;
		while(aRecordBoard[a][b]==jifang && temp<=3){ //temp用来记录向每个方向已经判断了多少步；
			m++; 
			yiwei(n,&a,&b);	temp+=1;
			if(a<1||a>15||b<1||b>15){
				liangduan1=1;
			...
			if(aRecordBoard[a][b]==0){
				...
				if(panduan_huosi(a,b,jifang)==1){	//在空位上填上己方的子，观察是否为活四
					chenghuosi+=1;
				}
				aRecordBoard[a][b]=0;
				yiwei(n,&a,&b);	temp+=1;
		...
		if(m==3 && liangduan1==0 && liangduan2==0 && kong<2 && chenghuosi>=1){	//两端为空，并且中间最多有一个空白位置，则为活三；
			huosan+=1;
			if(huosan==2){
				return 1;
			}
...
int sisijinshou(int a, int b,int c)	//判断四四禁手；
{
	...
	while(n!=5){
		...
		while(aRecordBoard[a][b]==jifang && temp<4.0){ //temp用来记录向每个方向已经判断了多少步，出现0.6是用来讨论出现空位置的情况；
			m++; 
		...
		n-=3; a=P; b=Q;	//以下是特殊的四四禁手的情况，即出现在一个方向上，不是在两个方向上的双四；
...
int panduan_huosi(int a,int b,int c)	//判断在（a，b）处下子，是否形成活四；
{
	int i,j,n=1,m,P,Q,huosi=0,chongsi=0,liangduan1,liangduan2;
	int jifang=0,duifang=0;
	float temp;
...
int panduan_chongsi(int a,int b,int c)	//判断在（a，b）处下子，是否形成冲四；
{
...
int panduan_huosan(int a,int b,int c)	//判断在（a，b）处下子，是否形成活三；
{
	...
	while(n!=5){ 
		...
		if(m==3 && liangduan1==0 && liangduan2==0 && kong<2 && chenghuosi>=1){
			huosan+=1;
		}
		n-=3; a=P; b=Q;
	}
...
int panduan_huoer(int a, int b,int c)	//判断是否形成活二；
{
	int n=1,m,P,Q,l,liangduan1,liangduan2,temp;	//l是用来记录连续的子数；
...
void defence(void)	//电脑执白棋，为保守方；
{
	...
	int a1[SIZE+1][SIZE+1],a2[SIZE+1][SIZE+1],a11[SIZE+1][SIZE+1],a12[SIZE+1][SIZE+1],a111,a112;
	int jizhi_x1=1,jizhi_y1=1,jizhi_x2=1,jizhi_y2=1;
	x_c=1;	y_c=1;
	for(i=1;i<=SIZE;i++){
		for(j=1;j<=SIZE;j++){
			c1[i][j]=0;	e1[i][j]=0;	d1[i][j]=0;	a1[i][j]=0;	a2[i][j]=0;	a11[i][j]=0;	a12[i][j]=0;
		}
	}
	for(x_c=1;x_c<=SIZE;x_c++){	//扫描整个棋盘，给没有子的位置评分；
		for(y_c=1;y_c<=SIZE;y_c++){
			if(aRecordBoard[x_c][y_c]==0){
				a1[x_c][y_c]=defence_score(x_c,y_c,2);	//计算此处落下己方子的得分；
				a2[x_c][y_c]=attack_score(x_c,y_c,1);		//计算此处落下对方子的得分；
			}
			...
			if(c1[x_c][y_c]>c2 || (c1[x_c][y_c]==c2 && a1[x_c][y_c]==a2[jizhi_x1][jizhi_y1])){
				c2=c1[x_c][y_c];	x_current1=x_c;	y_current1=y_c;	jizhi_x1=x_c;	jizhi_y1=y_c;
				if(c1[x_c][y_c]>=400000 || c1[x_c][y_c]==-1 ){	//如果此时对方出现活三或比活三更紧急的情况，或者出现禁手，不再向下看对方的落子位置；
	...
	while(temp1<1){		//向后预测对方的落子位置；
		...
		for(x_c=1;x_c<=SIZE;x_c++){
			for(y_c=1;y_c<=SIZE;y_c++){
				...
				if(d1[x_c][y_c]>d2 || (d1[x_c][y_c]==d2 && a11[x_c][y_c]==a12[jizhi_x2][jizhi_y2])){	//如果两处评分相等，但是攻击的分数大于防守的分数就立刻就选择进攻位下子
					d2=d1[x_c][y_c];	x_current2=x_c;	y_current2=y_c;	jizhi_x2=x_c;	jizhi_y2=y_c;
...
void attack(void)	//电脑执黑棋，为进攻方；
{
	...
	for(x_c=1;x_c<=SIZE;x_c++){		//扫描整个棋盘，给没有子的位置评分；
		for(y_c=1;y_c<=SIZE;y_c++){
			if(aRecordBoard[x_c][y_c]==0){
				a1[x_c][y_c]=defence_score(x_c,y_c,1);	//计算此处落下己方子的得分；
				a2[x_c][y_c]=attack_score(x_c,y_c,2);		//计算此处落下对方子的得分；
			}
			else if(aRecordBoard[x_c][y_c]==1)
				;
			else if(aRecordBoard[x_c][y_c]==2)
			...
			if(c1[x_c][y_c]>c2){
				c2=c1[x_c][y_c];	x_current1=x_c;	y_current1=y_c;	jizhi_x1=x_c;	jizhi_y1=y_c;
				if(c1[x_c][y_c]>=400000 || c1[x_c][y_c]==-1){	//如果此时对方出现活三或比活三更紧急，或者为禁手等情况，不再向下看对方的落子位置；
					x_max=&x_current1;	y_max=&y_current1;
					temp1=1;	temp2=1;	reset=0;
				}
				printf("c[%d][%d]=%d c2=%d \n",x_c,y_c,c1[x_c][y_c],c2);
...
int defence_score(int a,int b,int c)	//己方子的防守得分；
{
	...
	if(c==1 && ((panduan_huosi(a,b,c)==1 || panduan_chongsi(a,b,c)==1)&& panduan_huosan(a,b,c)==1)){
		dscore+=600000;			//出现三、四的情况；
	}else if(c==2 && ((panduan_huosi(a,b,c)==1 || panduan_chongsi(a,b,c)==1)&& panduan_huosan(a,b,c)>=1)){
		dscore+=600000;
	}
...
int attack_score(int a,int b,int c)		//用对方子的进攻得分，评分方式与defence_score相同；
{
	int dscore=0;
	aRecordBoard[a][b]=c;
	if(c==1 && win(a,b,c)){
